...
sub makeNotebook {
    ...
    my $tran = $n->new_ttk__frame;
    $self->makeTransactionFrame($tran);
    ...
    $n->add($tran, -text => "Confirm transactions");
...
sub makeTransactionFrame {
    my ($self, $parent) = @_;
    my $tranFrame = TableView::Transactions->new($parent, $self->{controller});
    $tranFrame->g_grid();
}
